...
from bootcamp.core.forms import DeactivateAccountForm, ChangePasswordForm, ProfileForm
...
def deactivate_account(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # provide notification to user group
            exit_post = '{0} has left the network.'.format(user.username)
            feed = Feed(user=user, post=exit_post)
            feed.save()
            
            #messages.add_message(request,
            #                     messages.SUCCESS,
            #                     'Your profile was successfully inactivated.')
            return redirect('deactivated') # TODO: better redirection page

    else:
        form = DeactivateAccountForm(instance=user)

    return render(request, 'core/deactivate_account.html', {'form': form})
